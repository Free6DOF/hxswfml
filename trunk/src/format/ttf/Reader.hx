...
class Reader
{
	...
	function readNameTable(bytes:Bytes):Array<NameRecord>
	{
		...
		for(i in 0...count)
		{
			if(nameRecords[i].nameID==4 && (nameRecords[i].platformId==0 || nameRecords[i].platformId==1 || nameRecords[i].platformId==3))
			{
		...
		else
		{
			...
			switch(fontNameRecord.platformId)
			{
				...
				case 1, 2:// Macintosh encoding, ASCII// ISO encoding, ASCII
					for (i in 0...fontNameRecord.length)
						fontNameRecord.record+=String.fromCharCode(input.readByte());
			}
		}

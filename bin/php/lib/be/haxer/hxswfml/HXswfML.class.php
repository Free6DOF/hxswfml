...
class be_haxer_hxswfml_HXswfML {
	...
	public function createSWF($xml, $swfWriter) {
		...
		$tag = $»it->next();
		{
			...
			case "tween":{
				{
					$_g4 = 0; $_g14 = $this->tween();
					while($_g4 < $_g14->length) {
						$tag14 = $_g14[$_g4];
						++$_g4;
						$swfWriter->writeTag($tag14);
						unset($tag14);
					}
				}
			}break;
			...
			unset($tag14,$tag13,$tag12,$tag1,$_g4,$_g3,$_g2,$_g14,$_g13,$_g12,$_g1,$_g);
		}
		}
		$swfWriter->writeEnd();
	}
	...
	public function symbolClass() {
		...
		if($baseClass != "") {
			if($this->checkValidBaseClass($baseClass)) {
				...
				$tags = new _hx_array(array($this->createABC($className, $baseClass), format_swf_SWFTag::TSymbolClass($symbols)));
			}
			else {
	...
	public function setup() {
		...
		$this->validElements->set("defineabc", new _hx_array(array("file", "name")));
	...
	public function checkValidBaseClass($c) {
		{
	...
	function __toString() { return 'be.haxer.hxswfml.HXswfML'; }
}

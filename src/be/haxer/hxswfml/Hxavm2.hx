...
class Hxavm2
{
	...
	{	
		...
		for(_classNode in ctx_xml.elements())
		{
			...
			{
				default : 
					throw ('<'+_classNode.nodeName + '> Must be <function>, <init>, <import> or <class [<var>], [<method>]>.');
					...
					for(member in _classNode.elements())
					{
						switch(member.nodeName)
						{
							case 'field', 'var':
								...
								var const:Bool = member.get('const') == 'true';
								...
								ctx.defineField(name, ctx.type(getImport(type)), isStatic, _value, const, ns);

							case 'method', 'function':
								createFunction(member, 'method', cl.isInterface);

							default : 
								throw (member.nodeName + ' Must be <method/> or <var/>.');
						}
					}
	...
	function createFunction(node:Xml, functionType:String, ?isInterface:Bool=false)
	{
		...
		var result:Bool = writeCodeBlock(node, f);
		...
		for (o in member.elements())
		{
			var op = switch(o.nodeName)
			{
				...
				case	"ODxNs", "ODebugFile" :
						Type.createEnum(OpCode, o.nodeName, [ctx.string(getAnyAttribute(o, ['v', 'file']))]);
						
				case	"OString":
						Type.createEnum(OpCode, o.nodeName, [ctx.string(StringTools.urlDecode(getAnyAttribute(o, ['v', 'file'])))]);
				...
				case	"OClassDef":
						if (!classDefs.exists(o.get('c')))
							throw o.get('c') + ' must be created as class before referencing it here.';
						else
							Type.createEnum(OpCode, o.nodeName, [classDefs.get(o.get('c'))]);
	
				case	"OFunction":
						if (!functionClosures.exists(o.get('f')))
							throw o.get('f') + ' must be created as function (closure) before referencing it here.';
						else
							Type.createEnum(OpCode, o.nodeName, [functionClosures.get(o.get('f'))]);
				...
				case	"OGetSuper", "OSetSuper", "OGetDescendants", "OFindPropStrict", "OFindProp", "OFindDefinition", "OGetLex", "OSetProp", "OGetProp", "OInitProp", 
						...
						var v = getAnyAttribute(o, ['v', 'c', 'cast', 'p', 'd', 't']);
						if (v == '#arrayProp')
				...
				case	"OCallSuper", "OCallProperty", "OConstructProperty", "OCallPropLex", "OCallSuperVoid", "OCallPropVoid":
						var p = getAnyAttribute(o, ['type', 'name', 'p']);
				...
				case	"ORegKill", "OReg", "OIncrReg", "ODecrReg", "OIncrIReg", "ODecrIReg", "OSmallInt", "OInt", "OGetScope", "ODebugLine", "OBreakPointLine", "OUnknown", 
						...
						var v = Std.parseInt(getAnyAttribute(o, ['c', 'r', 'v', 'n', 's', 'line', 'byte', 'nargs', 'nfields', 'nvalues' ]));
				...
				case	"OCatch":
						var _try =
						{
							...
							type :  ctx.type(getImport(o.get('type'))),
							variable : ctx.type(getImport(o.get('variable')))
						}
						f.trys.push(_try);
				...
				case	"OSetReg":
						ctx.allocRegister();
						var v = Std.parseInt(getAnyAttribute(o, ['r', 'v']));
						Type.createEnum(OpCode, o.nodeName, [v]);
				...
				case 	"OOp" :
						Type.createEnum(OpCode, 'OOp', [Type.createEnum(Operation, getAnyAttribute(o,['type', 'op', 'args']))]);
												
				case	"OCallStatic":
						var meth:Index<MethodType> = Idx(Std.parseInt(o.get('meth')));// ctx.type(o.get('meth'));
						var nargs = Std.parseInt(o.get('nargs'));
						Type.createEnum(OpCode, o.nodeName, [meth, nargs]);//??
											
				case	"OCallMethod":
						Type.createEnum(OpCode, o.nodeName, [Std.parseInt(o.get('s')), Std.parseInt(o.get('nargs'))]);
				...
				case	"OJump":
						...
						else
						{
							...
							Type.createEnum(OpCode, o.nodeName, [j, offset]);
						}
				...
				case	"OSwitch":
						...
						Type.createEnum(OpCode, o.nodeName, [Std.parseInt(o.get('def')), deltas]);
												
				case	"ONext":
						Type.createEnum(OpCode, o.nodeName, [Std.parseInt(o.get('r1')), Std.parseInt(o.get('r2'))]);
	...
	private function logStack(msg:Dynamic)
	{
		trace(msg);
	}

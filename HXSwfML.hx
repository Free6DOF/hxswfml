...
class HXswfML
{
	...
	public function xml2swf(input : String, fileOut : String) : Bytes
	{
		...
		{
			error('!ERROR: Supported file formats for output are .swf and .swc');
			return null;
		}
	}
	...
	private function defineABC()
	{
		...
		var lookupStrings = ["Boot", "Lib", "Type"];//Void, Math, ...expand? custom classnames?
		for (tag in tags)
		{
			switch (tag)
			{
	...
	private function setup() : Void
	{
		...
		validChildren.set('swf', ['header', 'fileattributes', 'setbackgroundcolor', 'scriptlimits', 'definebitsjpeg', 'defineshape', 'definesprite', 'definebutton', 'definebinarydata', 'definesound', 'definefont', 'defineedittext', 'defineabc', 'definescalinggrid', 'placeobject', 'removeobject', 'startsound', 'symbolclass', 'metadata', "framelabel", 'showframe', 'endframe']);
		validChildren.set('defineshape', ['beginfill', 'begingradientfill', 'beginbitmapfill', 'linestyle', 'moveto', 'lineto', 'curveto', 'endfill', 'endline', 'clear', 'drawcircle', 'drawellipse', 'drawrect', 'drawroundrect', 'drawroundrectcomplex']);
		validChildren.set('definesprite', ['placeobject', 'removeobject', 'startsound', "framelabel", 'showframe', 'endframe', 'tween']);
		validChildren.set('definebutton', ['buttonstate']);
		validChildren.set('tween', ['tw']);
		...
		validElements.set('defineabc', ['file']);
